...
namespace PFE.Directeur.Simulations
{
    public partial class DetailsSim : System.Web.UI.Page
    {
        ...
        string xml_file = "E:\\pfe\\ProjetFinEtude\\Directeur\\Simulations\\DirSim\\simulation.xml";
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                zoneReclam.Visible = false;
                sucessReclam.Visible = false;
                envoyer.Visible = false;
                simValide.Visible = false;
                listeDetailsSimulations();
                simEstValide();
                SimTotal.Text = "TOTAL= " + getValeurSim()["NU_VALEUR"].ToString() + "TND";
                DesSim.Text = getDesSim();
            }
            else { }
        }
        // verifier si la simulation est valide ou pas
        public void simEstValide()
        {
            ...
            if (reader.Read())
            {
                if (reader["SIM_STATE"].ToString() == "UNVALIDATED") //si invalide
                {
                    //rendre le bouton de reclation et info visible
                    recInfo.Visible = true;
                    reclamer.Visible = true;

                }
                else if (reader["SIM_STATE"].ToString() == "VALIDATED") //else hide it
                {
                    simValide.Visible = true;
                    recInfo.Visible = false;
                    reclamer.Visible = false;
                    zoneReclam.Visible = false;
                    envoyer.Visible = false;
                    sucessReclam.Visible = false;
                }
        ...
        //recuperer la designation de la simulation
        public string getDesSim()
        {
            string desSim = "";
            string qry = "SELECT ST_DESIM FROM EXTSIMULATION WHERE ID_SIM=" + Convert.ToInt32(Request.QueryString["ID_SIM"]);
            SqlConnection con = new SqlConnection(connectionString);
            con.Open();
            SqlCommand cmd = new SqlCommand(qry, con);
            SqlDataReader reader = cmd.ExecuteReader();
        ...
        private void listeDetailsSimulations()
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                {
                    string sql = "SELECT ST_CODEQ, ST_DESEQ, ST_DESCH, CONVERT(varchar, DT_ENTREE, 1) AS DT_ENTREE, CONVERT(varchar, DT_SORTIE, 1) AS DT_SORTIE, CONCAT(NU_NBJRS, 'jours') NU_NBJRS, NU_TAUXLOC, NU_VALEUR FROM EXTDEPEQU WHERE ID_SIM = " + Convert.ToInt32(Request.QueryString["ID_SIM"]) + " ORDER BY DT_ENTREE ASC";
                    cmd.CommandText = sql;
                    cmd.Connection = con;
        ...
        // recuperer la valeur de la simulation
        public SqlDataReader getValeurSim()
        {
            SqlConnection con = new SqlConnection(connectionString);
            con.Open();
            string qry = "SELECT DT_CREATE, ID_DIR, NU_VALEUR, ST_DESCH FROM EXTSIMULATION WHERE ID_SIM =" + Convert.ToInt32(Request.QueryString["ID_SIM"]);
            SqlCommand cmd = new SqlCommand(qry, con);
            SqlDataReader reader = cmd.ExecuteReader();
            if (reader.Read())
            {
        ...
        //-----------------------------EXPORT DE LA SIMULATION EN FORMAT EXCEL ET PDF ---------------------------
        ...
        //--------------------CONTROLER LA ZONE DE RECLAMATION-------------------------

        protected void reclamer_Click(object sender, EventArgs e)
        {
            zoneReclam.Visible = true;
            reclamer.Visible = false;
            sucessReclam.Visible = false;
        ...
        {
            gvSimDetails.PageIndex = e.NewPageIndex;
            listeDetailsSimulations();
        }

        protected void envoyer_Click(object sender, EventArgs e)
        {
            string qry = "INSERT INTO EXTRECLAM (ID_CODUTI, ID_SIM, ST_RECLAM, DT_RECLAM) VALUES (@ID_CODUTI, @ID_SIM, @ST_RECLAM, @DT_RECLAM)";
            SqlConnection con = new SqlConnection(connectionString);
            SqlCommand cmd = new SqlCommand(qry, con);
            con.Open();
            ...
            cmd.Parameters.AddWithValue("@ST_RECLAM", Request.Form["reclamation"].ToString());
            cmd.Parameters.AddWithValue("@DT_RECLAM", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff"));
            cmd.ExecuteNonQuery();
            con.Close();
            sucessReclam.Visible = true;
            zoneReclam.Visible = false;
            envoyer.Visible = false;
            string TRACE = Session["UserName"] + " a reclamé sa simulation num " + Request.QueryString["ID_SIM"];
        ...
        protected void ExportPDF_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                invoice facture = new invoice(Server.MapPath("DirSim") + "\\simulation.xml");
                DataRow drHead = facture.GetInvoiceHeader().Rows[0];
                DataRow drCompany = facture.GetInvoicePayeeInfo().Rows[0];
                using (System.IO.FileStream fs = new FileStream(Server.MapPath("DirSim") + drHead["invoiceNumber"].ToString() + ".pdf", FileMode.Create))
                {
                    ...
                    top_margin -= 80;
                    left_margin = 350;
                    float total = Convert.ToSingle(Convert.ToDecimal(getValeurSim()["NU_VALEUR"].ToString()));
                    ...
                    writeText(cb, "", left_margin, top_margin - 24, f_cb, 10);
                    ...
                    MsgResult("la simulation est enregistrée dans le dossier DirSim!", "information");
                }
            }
