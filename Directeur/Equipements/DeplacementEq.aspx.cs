...
{
    public partial class DeplacementEq : System.Web.UI.Page
    {
        ...
        // lister les deplacements dans le chantiers appartenant au directeur connecté
        private void ListeDeplacements()
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                if (ddlChantier.SelectedItem.Text == "Tous les chantiers")
                {
                    ...
                    string sql = "SELECT * FROM EXTDEPEQU WHERE (MONTH(DT_ENTREE) = @MOIS) AND (YEAR(DT_ENTREE) = @YEAR) AND ST_DESCH IN (SELECT ST_DESCH FROM EXTCHANTIER WHERE ID_CODUTIDIR LIKE '" + Session["UserName"].ToString() + "') ORDER BY ST_DESCH";
                    cmd.CommandText = sql;
                    cmd.Connection = con;
