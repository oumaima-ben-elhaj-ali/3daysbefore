...
{
    public partial class ValiderSim : System.Web.UI.Page
    {
        ...
        public void SimEstValide()
        {
            ...
            while (reader.Read())
            {
                //si la simulation n'est pas encore valider alors l'admin peut effectuer qlq changement
        ...
        private void ListeSimulation()
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                {
                    string sql = "SELECT * FROM EXTDEPEQU WHERE ID_SIM=" + Convert.ToInt32(Request.QueryString["ID_SIM"]);
                    cmd.CommandText = sql;
                    cmd.Connection = con;
        ...
        void PopulateGridview()
        {
            ...
            {
                sqlCon.Open();
                string qry = "SELECT * FROM EXTDEPEQU WHERE ID_SIM =" + Convert.ToInt32(Request.QueryString["ID_SIM"]);
        ...
        //export la simulation en format PDF
        /*------------------------------------------------------------------------------------------------------------------------
        ...
        protected void ExportPDF_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (System.IO.FileStream fs = new FileStream(Server.MapPath("AdmSim") + drHead["invoiceNumber"].ToString() + ".pdf", FileMode.Create))
                {
                    ...
                    cb.EndText();
                    cb.SetLineWidth(0f);
                    cb.MoveTo(30, top_margin - 200);
                    cb.LineTo(150, top_margin - 200);
                    cb.Stroke();
                    cb.BeginText();
        ...
        protected void Valider_Click(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(connectionString);
            con.Open();
            string query = "UPDATE EXTRECLAM SET EST_TRAITEE=1 WHERE ID_SIM=" + Convert.ToInt32(Request.QueryString["ID_SIM"]);
            SqlCommand sqlCmd = new SqlCommand(query, con);
            sqlCmd.ExecuteNonQuery();
            UpdateSinStatus();
            con.Close();
            Response.Redirect("ReclamSim.aspx");
        }
        // update SIM_STATE='VALIDATED' 
        public void UpdateSinStatus()
        {
            SqlConnection con = new SqlConnection(connectionString);
            con.Open();
            string query = "UPDATE EXTSIMULATION SET SIM_STATE='VALIDATED' WHERE ID_SIM=" + Convert.ToInt32(Request.QueryString["ID_SIM"]);
            SqlCommand sqlCmd = new SqlCommand(query, con);
            sqlCmd.ExecuteNonQuery();
            con.Close();
        }
