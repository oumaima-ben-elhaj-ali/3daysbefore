...
{
    public partial class DetailsFac : System.Web.UI.Page
    {
        ...
        string xml_file = "E:\\pfe\\ProjetFinEtude\\Admin\\Factures\\AdmFactures\\facture.xml";
        protected void Page_Load(object sender, EventArgs e)
        {
            BindNotifRepeater();
            string strSql;

            float TVA = CalculTvaTtc(Convert.ToSingle(getFactValue()))[0];
            float TTC = CalculTvaTtc(Convert.ToSingle(getFactValue()))[1];
            factTotal.Text = "TOTAL FACTURE : " + getFactValue().ToString();
            DataSet ds = new DataSet();
            //selectionner les simulations validees correspondantes a l'ID facture recu  
        ...
        // recuperer les details de la facture : les locations 
        ...
        protected void ExportPDF_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                invoice facture = new invoice(Server.MapPath("AdmFactures") + "\\facture.xml");
                DataRow drHead = facture.GetInvoiceHeader().Rows[0];
                DataRow drCompany = facture.GetInvoicePayeeInfo().Rows[0];
                using (System.IO.FileStream fs = new FileStream(Server.MapPath("AdmFactures") + drHead["invoiceNumber"].ToString() + ".pdf", FileMode.Create))
                {
                    ...
                    top_margin -= 80;
                    left_margin = 350;
                    float total = Convert.ToSingle(getFactValue());
                    ...
                    MsgResult("Votre Facture est enregistrée dans le dossier AdmFactures!", "information");
                }
            }
